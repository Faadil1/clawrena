...
type ReceiptRow = {
  ...
  replayKey?: string;
  authorityState?: "AUTHORIZED" | "REFUSED" | "ABSTAINED" | "PREPARED";
    ...
    <div className="fw-page">
      <div className="fw-wrap">
        ...
        <div className="fw-sheet mt-[18px]">
          <div className="fw-sheet-head"><div className="fw-sheet-title"><span className="fw-sheet-no">R</span><div><h2>Decision receipt ledger</h2><p>P11 receipts carry a versioned Evidence Passport so a judge or another agent can tell which policy produced the decision and what would have to change before authority is reconsidered.</p></div></div><span className="fw-badge">{receipts.length} RECENT</span></div>
          ...
          : <><div className="fw-ledger-head"><span>Observed</span><span>Decision</span><span>Evidence / reason</span><span>Budget</span></div><div className="fw-ledger">{receipts.map((r) => <div key={r._id} className="fw-ledger-row"><span className="font-mono text-[9px] text-ink-faint">{timeAgo(r.createdAt)}</span><span><span className={`fw-state ${r.decision === "reject" ? "fw-state--flag" : r.decision === "execute" ? "fw-state--pass" : "fw-state--unknown"}`}>{r.authorityState ?? r.decision.toUpperCase()}</span><span className="block mt-1 font-mono text-[8px] text-ink-faint">{r.executionMode.toUpperCase()}</span></span><span className="min-w-0"><span className="flex items-baseline gap-2"><b className="text-[12px]">SCORE {r.score ?? "—"}/100</b><span className="font-mono text-[9px] text-ink-faint">{shorten(r.tokenMint)}</span></span><span className="block mt-1 text-[11px] text-ink-mid leading-snug">{r.reasons.join(" · ") || "No reason recorded"}</span>{r.unknowns.length > 0 && <span className="block mt-1 text-[10px] text-[#b63d36]">UNKNOWN → {r.unknowns.join(", ")}</span>}{r.policyVersion && <span className="block mt-2 font-mono text-[8px] uppercase tracking-[.08em] text-ink-faint">POLICY {r.policyVersion}{r.replayKey ? ` · REPLAY ${r.replayKey}` : ""}</span>}{r.freshnessExpiresAt && <span className="block mt-1 font-mono text-[8px] uppercase tracking-[.08em] text-ink-faint">FRESH UNTIL {new Date(r.freshnessExpiresAt).toISOString()}</span>}{(r.counterfactuals?.length ?? 0) > 0 && <span className="block mt-2 text-[10px] text-ink-mid">TO RECONSIDER → {r.counterfactuals?.join(" · ")}</span>}{r.txSignature && <span className="block mt-1 font-mono text-[9px] text-ink-faint">TX {shorten(r.txSignature)}</span>}</span><span className="font-mono text-[10px] text-right">{r.riskBudgetSol !== undefined ? formatSol(r.riskBudgetSol) : "—"}</span></div>)}</div></>}
        </div>
        ...
        <footer className="fw-footer-rule"><span>observed ≠ inferred</span><span>pending ≠ verified</span><span>rejects stay in record</span><span>policy is replayable</span><span>real failure &gt; fake success</span></footer>
      </div>
    </div>
  );
}
