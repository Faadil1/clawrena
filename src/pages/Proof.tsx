...
type ReceiptRow = {
  ...
  replayKey?: string;
  policyState?: "QUALIFIED" | "REFUSED" | "ABSTAINED" | "PREPARED";
  ...
  return (
    <div className="fw-page">
      <div className="fw-wrap">
        ...
        <section className="fw-proof-banner"><div className="fw-proof-banner-main"><div className="fw-label">CLAIM AUTHORITY</div><h2>No receipt → no claim.</h2><p>QUALIFIED means the evidence gate passed. It does not move value: last-mile risk/provider checks still control execution. Verified on-chain volume requires a stored transaction signature and an independently persisted confirmation slot.</p></div><div className="fw-proof-banner-state"><div><b>{receipts.length > 0 ? "RECORDED" : "LOCKED"}</b><span>{receipts.length > 0 ? "EVIDENCE EXISTS" : "AWAITING RUNTIME"}</span></div></div></section>
        ...
        <section className="fw-boundary"><Boundary label="01 / OBSERVED" title="Facts enter" body="Live Jupiter price/liquidity, Solana owner concentration, launch timestamp and provider request IDs." /><Boundary label="02 / QUALIFIED" title="Evidence can pass" body="Passing evidence may proceed to last-mile provider/risk preflight. QUALIFIED is not execution authorization." /><Boundary label="03 / PROVED" title="Claims leave" body="Only separately verified execution evidence can become verified on-chain volume." /></section>

        <div className="fw-sheet mt-[18px]">
          <div className="fw-sheet-head"><div className="fw-sheet-title"><span className="fw-sheet-no">R</span><div><h2>Decision receipt ledger</h2><p>P11 receipts carry a versioned Evidence Passport so a judge or another agent can tell which policy produced the decision and what would have to change before a refusal is reconsidered.</p></div></div><span className="fw-badge">{receipts.length} RECENT</span></div>
          ...
          : <><div className="fw-ledger-head"><span>Observed</span><span>Decision</span><span>Evidence / reason</span><span>Budget</span></div><div className="fw-ledger">{receipts.map((r) => <div key={r._id} className="fw-ledger-row"><span className="font-mono text-[9px] text-ink-faint">{timeAgo(r.createdAt)}</span><span><span className={`fw-state ${r.decision === "reject" ? "fw-state--flag" : r.decision === "execute" ? "fw-state--pass" : "fw-state--unknown"}`}>{r.policyState ?? r.decision.toUpperCase()}</span><span className="block mt-1 font-mono text-[8px] text-ink-faint">{r.executionMode.toUpperCase()}</span></span><span className="min-w-0"><span className="flex items-baseline gap-2"><b className="text-[12px]">SCORE {r.score ?? "—"}/100</b><span className="font-mono text-[9px] text-ink-faint">{shorten(r.tokenMint)}</span></span><span className="block mt-1 text-[11px] text-ink-mid leading-snug">{r.reasons.join(" · ") || "No reason recorded"}</span>{r.unknowns.length > 0 && <span className="block mt-1 text-[10px] text-[#b63d36]">UNKNOWN → {r.unknowns.join(", ")}</span>}{r.policyVersion && <span className="block mt-2 font-mono text-[8px] uppercase tracking-[.08em] text-ink-faint">POLICY {r.policyVersion}{r.replayKey ? ` · REPLAY ${r.replayKey}` : ""}</span>}{r.freshnessExpiresAt && <span className="block mt-1 font-mono text-[8px] uppercase tracking-[.08em] text-ink-faint">FRESH UNTIL {new Date(r.freshnessExpiresAt).toISOString()}</span>}{(r.counterfactuals?.length ?? 0) > 0 && <span className="block mt-2 text-[10px] text-ink-mid">TO RECONSIDER → {r.counterfactuals?.join(" · ")}</span>}{r.txSignature && <span className="block mt-1 font-mono text-[9px] text-ink-faint">TX {shorten(r.txSignature)}</span>}</span><span className="font-mono text-[10px] text-right">{r.riskBudgetSol !== undefined ? formatSol(r.riskBudgetSol) : "—"}</span></div>)}</div></>}
        </div>
        ...
        <footer className="fw-footer-rule"><span>observed ≠ inferred</span><span>qualified ≠ authorized</span><span>pending ≠ verified</span><span>rejects stay in record</span><span>real failure &gt; fake success</span></footer>
      </div>
    </div>
  );
}
